...
export const SUPPORTED_ETHERSCAN_NETWORKS = ['main', 'ropsten', 'kovan', 'goerli', 'matic'];
...
export const verifyContract = async (
  ...
  try {
    ...
    fs.writeSync(paramsFile.fd, `module.exports = ${JSON.stringify([...constructorArguments])};`);

    // Write a temporal file for library args
    const librariesFile = await file({
      prefix: 'verify-libraries-',
      postfix: '.js',
    });
    fs.writeSync(librariesFile.fd, `module.exports = ${libraries};`);

    const params = {
      address: address,
      libraries: librariesFile.path,
      constructorArgs: paramsFile.path,
      relatedSources: true,
    };
    await runTaskWithRetry('verify', params, times, msDelay, () => {
      paramsFile.cleanup();
      librariesFile.cleanup();
    });
  } catch (error) {}
};
